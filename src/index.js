...
const DEFAULT_TRIAL_CONFIG = { is_active: 1, button_text: "🎁 دریافت اکانت رایگان (تست)", first_time_days: 3, repeat_days: 2, cooldown_days: 30, is_single: 1, trial_id: 1 };
const DEFAULT_FESTIVAL_CONFIG = { is_active: 0, button_text: "🎉 جشنواره ویژه", days: 1, is_single: 1, start_date: "", end_date: "", festival_id: 1 };
...
// متن توضیحات آموزشیِ همراه هر ویدیو (جداگانه برای V2ray و V2box) که به‌صورت کپشن ارسال می‌شود.
// به‌صورت پیش‌فرض، مراحل دقیق «به‌روزرسانی اشتراک» را مخصوص همان اپلیکیشن (در صورت افت سرعت یا عدم استفاده طولانی) توضیح می‌دهد.
// از داخل پنل ادمین (⚙️ تنظیمات ربات ← 🎥 مدیریت ویدیوهای آموزشی) به‌طور جداگانه برای هر اپ قابل ویرایش است.
const DEFAULT_TUTORIAL_DESC = {
  v2ray: "کاربر گرامی؛\n\nدر صورتی که سرعت VPN شما پایین بود یا مدتی است از آن استفاده نکرده‌اید، لازم است اشتراک خود را در برنامهٔ V2rayNG به‌روزرسانی نمایید. برای این کار:\n\n1️⃣ وارد برنامه V2rayNG شوید.\n2️⃣ روی آیکون سه‌نقطه (⋮) در گوشهٔ بالا سمت راست صفحه بزنید.\n3️⃣ گزینهٔ «Update subscription» (به‌روزرسانی اشتراک) را انتخاب کنید.\n4️⃣ چند ثانیه صبر کنید تا سرورهای جدید بارگذاری شوند و سپس به بهترین سرور متصل شوید.\n\nنیازی به دریافت کانفیگ جدید یا حذف و نصب مجدد برنامه نیست؛ همین چند مرحله کافی است.\n\nدر صورت بروز هرگونه مشکل، از بخش «📞 ارتباط با پشتیبانی» با ما در تماس باشید.",
  v2box: "کاربر گرامی؛\n\nدر صورتی که سرعت VPN شما پایین بود یا مدتی است از آن استفاده نکرده‌اید، لازم است اشتراک خود را در برنامهٔ V2Box به‌روزرسانی نمایید. برای این کار:\n\n1️⃣ وارد برنامه V2Box شوید.\n2️⃣ از منوی پایین صفحه، وارد بخش «Configs» شوید.\n3️⃣ روی علامت «+» در بالای صفحه بزنید.\n4️⃣ گزینهٔ «Update All Subscriptions» را انتخاب کنید تا همهٔ اشتراک‌ها به‌روزرسانی شوند.\n\nنیازی به دریافت کانفیگ جدید یا حذف و نصب مجدد برنامه نیست؛ همین چند مرحله کافی است.\n\nدر صورت بروز هرگونه مشکل، از بخش «📞 ارتباط با پشتیبانی» با ما در تماس باشید."
};

async function getTutorialDescConfig(db) {
  return await getSetting(db, 'tutorial_desc_config', DEFAULT_TUTORIAL_DESC);
}

async function getTutorialDesc(db, type) {
  const cfg = await getTutorialDescConfig(db);
  return cfg[type] || DEFAULT_TUTORIAL_DESC[type] || "";
}

async function setTutorialDesc(db, type, text) {
  const cfg = await getTutorialDescConfig(db);
  cfg[type] = text;
  await setSetting(db, 'tutorial_desc_config', cfg);
}
...
async function renderTutorialVideoMenu(db) {
  ...
  const descCfg = await getTutorialDescConfig(db);
  ...
  const msg = `🎥 <b>مدیریت ویدیوهای آموزشی</b>\n\n` +
    ...
    `📝 <b>متن توضیحاتِ همراهِ ویدیوی V2ray:</b>\n<i>${escapeHtml(descCfg.v2ray)}</i>\n\n` +
    `📝 <b>متن توضیحاتِ همراهِ ویدیوی V2box:</b>\n<i>${escapeHtml(descCfg.v2box)}</i>\n\n` +
    `👇 برای آپلود/جایگزینی ویدیو یا ویرایش متن توضیحات هر اپ، روی دکمه مربوطه بزنید.`;
  const kb = {
    inline_keyboard: [
      ...
      [{ text: "📝 ویرایش توضیحات v2ray", callback_data: "admtutdesc_edit_v2ray" }, { text: "📝 ویرایش توضیحات v2box", callback_data: "admtutdesc_edit_v2box" }]
    ]
  };
  return { msg, kb };
}
...
async function renderTrialConfigMessage(db) {
  ...
  const msg = `🎁 <b>تنظیمات اکانت تست</b>\n\n` +
    `🆔 آیدی دوره فعلی: <b>#${cfg.trial_id || 1}</b>\n` +
    ...
    `ℹ️ توجه: اکانت تست و جشنواره یک سقف مشترک دارند؛ اگر کاربری اخیراً از جشنواره استفاده کرده باشد، تا پایان فاصله «${cfg.cooldown_days} روزه» بالا نمی‌تواند اکانت تست هم دریافت کند (و برعکس).`;
  const kb = { inline_keyboard: [
    ...
    [{ text: "🆕 شروع دوره جدید تست (ریست برای همه کاربران)", callback_data: "admtrial_new_round_ask" }],
...
async function renderFestivalConfigMessage(db) {
  ...
  const msg = `🎉 <b>تنظیمات جشنواره</b>\n\n` +
    `🆔 آیدی جشنواره فعلی: <b>#${cfg.festival_id || 1}</b>\n` +
    `🔘 وضعیت: ${cfg.is_active ? '✅ فعال' : '🚫 غیرفعال'}\n` +
    `✏️ متن دکمه: ${escapeHtml(cfg.button_text)}\n` +
    `🗓 مدت اعتبار اکانت: <b>${cfg.days}</b> روز\n` +
    `📅 بازه زمانی نمایش: ${windowInfo}\n` +
    `👥 نوع اکانت صادرشده: ${isSingle ? '👤 تک‌کاربره' : '👥 چندکاربره'}\n\n` +
    `ℹ️ هر کاربر از <b>جشنواره با همین آیدی (#${cfg.festival_id || 1})</b> فقط یک‌بار برای همیشه می‌تواند استفاده کند. برای این‌که همه کاربران دوباره بتوانند از یک جشنواره جدید استفاده کنند، از دکمه «شروع جشنواره جدید» زیر استفاده کنید (آیدی عوض می‌شود).\n` +
    `⏳ ضمناً استفاده از جشنواره، اکانت تست را هم تا پایان فاصله مجاز تست (تنظیمات اکانت تست) قفل می‌کند و برعکس.`;
  const kb = { inline_keyboard: [
    ...
    [{ text: "🆕 شروع جشنواره جدید (آیدی جدید)", callback_data: "admfestival_new_round_ask" }],
    [{ text: "🔙 بازگشت", callback_data: "admtrialfestival_menu" }]
  ] };
  return { msg, kb };
}
...
function wipeOptionsMenu() {
  return { inline_keyboard: [
    ...
    [{ text: "🎁 پاک کردن تاریخچه تست (همه کاربران)", callback_data: "admwipe_ask_test_history" }, { text: "🎉 پاک کردن تاریخچه جشنواره (همه کاربران)", callback_data: "admwipe_ask_festival_history" }],
...
const WIPE_TARGET_LABELS = {
  ...
  test_history: "تاریخچه اکانت تست همه کاربران (امکان دریافت دوباره تست/جشنواره برای همه فراهم می‌شود)",
  festival_history: "تاریخچه جشنواره همه کاربران (امکان دریافت دوباره همین جشنواره برای همه فراهم می‌شود)",
...
async function executeWipe(db, target) {
  ...
  } else if (target === 'test_history') {
    await db.prepare("UPDATE users SET last_test_date = NULL, last_test_id = NULL").run();
  } else if (target === 'festival_history') {
    await db.prepare("UPDATE users SET last_festival_claim = NULL, last_festival_id = NULL").run();
  } else if (target === 'all') {
    await db.prepare("DELETE FROM services").run();
    await db.prepare("DELETE FROM users").run();
    await db.prepare("DELETE FROM user_states").run();
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (update.message) {
        ...
        // قفل مشابه بالا، اما مخصوص «درخواست شارژ کیف پول» (جدول wallet_charges به‌جای فیلدهای سرویس)
        if (state && state.step === 'PENDING_ADMIN_WALLET' && user_id !== ADMIN_ID) {
          if (text === "❌ لغو عملیات") {
             if (state.admin_message_id) {
                 await callTelegram('editMessageReplyMarkup', { chat_id: ADMIN_ID, message_id: state.admin_message_id, reply_markup: { inline_keyboard: [] } });
                 await callTelegram('editMessageCaption', { chat_id: ADMIN_ID, message_id: state.admin_message_id, caption: "❌ <b>این درخواست شارژ کیف پول توسط کاربر لغو شد.</b>", parse_mode: "HTML" });
             }
             if (state.charge_id) {
                 // وضعیت را در دیتابیس هم به‌روزرسانی می‌کنیم تا اگر ادمین با تاخیر روی دکمه‌های قبلی بزند، دوباره تایید/رد نشود
                 await db.prepare("UPDATE wallet_charges SET status = 'REJECTED', decided_at_shamsi = ? WHERE id = ? AND status = 'PENDING'").bind(getShamsiNow(), state.charge_id).run();
             }
             await clearState(db, user_id);
             await sendMessage(chat_id, "✅ درخواست شارژ کیف پول شما لغو شد و به منوی اصلی بازگشتید.", await mainMenu(db, user_id));
          } else if (!msg.photo) {
             await sendMessage(chat_id, "⏳ <b>درخواست شارژ کیف پول شما در حال بررسی است!</b>\nلطفاً تا پاسخ ادمین صبور باشید یا دکمه «❌ لغو عملیات» را بزنید.", pendingMenu());
          }
          if (text === "❌ لغو عملیات") return new Response('OK');
        }
        ...
        if (text === "🚀 آموزش برنامه v2ray برای نصب کانفیگ") {
          ...
          const tutDescV2ray = await getTutorialDesc(db, 'v2ray');
          const captionV2ray = `🎥 آموزش نصب و راه‌اندازی کانفیگ در برنامه V2ray\n\n${tutDescV2ray}`;
          const res = await fetch(`https://api.telegram.org/bot${TOKEN}/sendVideo`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: chat_id, video: tvV2ray.file_id, caption: captionV2ray })
          });
        ...
        if (text === "📥 آموزش برنامه v2box برای نصب کانفیگ") {
          ...
          const tutDescV2box = await getTutorialDesc(db, 'v2box');
          const captionV2box = `🎥 آموزش نصب و راه‌اندازی کانفیگ در برنامه V2box\n\n${tutDescV2box}`;
          const res = await fetch(`https://api.telegram.org/bot${TOKEN}/sendVideo`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: chat_id, video: tvV2box.file_id, caption: captionV2box })
          });
          if (!res.ok) await sendMessage(chat_id, "❌ ویدیوی آموزشی یافت نشد یا مشکلی در ارسال وجود دارد. لطفاً به پشتیبانی اطلاع دهید.");
          return new Response('OK');
        }
        ...
        if (user_id !== ADMIN_ID) {
          ...
          if (trialCfgCheck.is_active && text === trialCfgCheck.button_text) {
            ...
            if (userRow && userRow.last_test_date) {
              ...
              if (diffDays < cooldownDays) {
                ...
                await sendMessage(user_id, `❌ <b>دریافت مجدد امکان‌پذیر نیست!</b>\n\nشما اخیراً از یک هدیه رایگان (اکانت تست یا جشنواره) استفاده کرده‌اید. دریافت هدیه رایگان بعدی هر <b>${cooldownDays} روز یکبار</b> امکان‌پذیر است.\n⏳ لطفاً <b>${remainingDays} روز</b> دیگر مراجعه کنید.`);
                return new Response('OK');
              }
              isFirstTime = false;
            }
            ...
            const sameDaysForBothTimes = Number(trialCfgCheck.first_time_days) === Number(trialCfgCheck.repeat_days);
            let msgText = "";

            if (isFirstTime) {
                msgText = `🎁 <b>مژده:</b> چون بار اول شماست، یک اکانت تست <b>${testDays} روزه (${trialUserTypeLabel})</b> به شما تعلق می‌گیرد!`;
                if (!sameDaysForBothTimes) {
            ...
            const admText = `🎁 <b>درخواست تست (${newState.type})</b>\n👤 کاربر: ${userLink}\n🆔 آیدی: <code>${user_id}</code>\n🆔 آیدی دوره تست: #${trialCfgCheck.trial_id || 1}\n💰 <b>قیمت:</b> رایگان${workerText}`;
            const admKb = { inline_keyboard: [
          ...
          if (isFestivalWindowOpen(festivalCfgCheck) && text === festivalCfgCheck.button_text) {
            ...
            const currentFestivalId = festivalCfgCheck.festival_id || 1;
            const userRow = await db.prepare("SELECT last_test_date, last_festival_id FROM users WHERE user_id = ?").bind(user_id).first();

            // هر جشنواره (بر اساس آیدی) فقط یک‌بار برای همیشه قابل استفاده است
            if (userRow && userRow.last_festival_id === currentFestivalId) {
              await sendMessage(user_id, "❌ <b>شما قبلاً از این جشنواره استفاده کرده‌اید!</b>\n\nاین هدیه فقط یک‌بار برای هر کاربر قابل استفاده است. در صورت برگزاری جشنواره جدید، مجدداً می‌توانید استفاده کنید.");
              return new Response('OK');
            }

            // سقف مشترک با اکانت تست: اگر اخیراً از تست یا جشنواره استفاده کرده، تا پایان فاصله مجاز تست باید صبر کند
            if (userRow && userRow.last_test_date) {
              const lastFreeTime = new Date(userRow.last_test_date).getTime();
              const diffDaysFestival = (Date.now() - lastFreeTime) / (1000 * 60 * 60 * 24);
              const cooldownDaysFestival = trialCfgCheck.cooldown_days || 30;
              if (diffDaysFestival < cooldownDaysFestival) {
                const remainingDaysFestival = Math.ceil(cooldownDaysFestival - diffDaysFestival);
                await sendMessage(user_id, `❌ <b>دریافت مجدد امکان‌پذیر نیست!</b>\n\nشما اخیراً از یک هدیه رایگان (اکانت تست یا جشنواره) استفاده کرده‌اید. دریافت هدیه رایگان بعدی هر <b>${cooldownDaysFestival} روز یکبار</b> امکان‌پذیر است.\n⏳ لطفاً <b>${remainingDaysFestival} روز</b> دیگر مراجعه کنید.`);
                return new Response('OK');
              }
            }
        ...
        if (msg.photo && state && state.step === 'WAIT_WALLET_RECEIPT' && user_id !== ADMIN_ID) {
          ...
          // به‌جای پاک کردن کامل state، کاربر را در یک «حالت انتظار» واقعی نگه می‌داریم
          // (مشابه سایر جریان‌ها) تا بتواند تا پاسخ ادمین درخواستش را لغو کند.
          const walletPendingState = { step: 'PENDING_ADMIN_WALLET', charge_id: chargeId, charge_amount: amount };
          if (adminMsgRes && adminMsgRes.ok) {
            walletPendingState.admin_message_id = adminMsgRes.result.message_id;
          }
          await setState(db, user_id, walletPendingState);

          await sendMessage(chat_id, "✅ رسید شارژ کیف پول شما با موفقیت ارسال شد و در صف بررسی قرار گرفت. پس از تایید ادمین، مبلغ به موجودی شما اضافه خواهد شد.", pendingMenu());
          return new Response('OK');
        }
        ...
        if (user_id === ADMIN_ID && state && state.step === 'WAIT_DOMAIN') {
          ...
          if (cfRes.success && cfRes.subLink) {
            ...
            if (state.action === 'test') {
              const trialCfgApply = await getTrialConfig(db);
              await db.prepare("INSERT INTO users (user_id, last_test_date, last_test_id) VALUES (?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET last_test_date = excluded.last_test_date, last_test_id = excluded.last_test_id").bind(state.target_user, getShamsiDateOnly(), trialCfgApply.trial_id || 1).run();
            } else if (state.action === 'festival') {
              const festivalCfgApply = await getFestivalConfig(db);
              // جشنواره سقف مشترک با تست دارد: last_test_date هم به‌روزرسانی می‌شود تا فاصله مجاز مشترک اعمال شود
              await db.prepare("INSERT INTO users (user_id, last_test_date, last_festival_claim, last_festival_id) VALUES (?, ?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET last_test_date = excluded.last_test_date, last_festival_claim = excluded.last_festival_claim, last_festival_id = excluded.last_festival_id").bind(state.target_user, getShamsiDateOnly(), getShamsiDateOnly(), festivalCfgApply.festival_id || 1).run();
            }
        ...
        // دریافت متن جدید توضیحاتِ همراهِ ویدیوی آموزشی (v2ray یا v2box) از ادمین
        if (user_id === ADMIN_ID && state && state.step === 'WAIT_TUTORIAL_DESC') {
          const descType = state.desc_type === 'v2box' ? 'v2box' : 'v2ray';
          const descLabel = descType === 'v2ray' ? 'V2ray' : 'V2box';
          const newDesc = (text || "").trim();
          if (!newDesc) {
            await sendMessage(chat_id, "⚠️ لطفاً یک متن معتبر ارسال کنید. برای انصراف، دکمه «❌ لغو عملیات» را بزنید.", pendingMenu());
            return new Response('OK');
          }
          if (newDesc.length > 900) {
            await sendMessage(chat_id, `⚠️ متن ارسالی خیلی طولانی است (${newDesc.length} کاراکتر). به‌دلیل محدودیت کپشن تلگرام (۱۰۲۴ کاراکتر که بخشی از آن هم به عنوان تیتر آموزش استفاده می‌شود)، لطفاً متن را کوتاه‌تر کرده و دوباره ارسال کنید.`, pendingMenu());
            return new Response('OK');
          }
          await setTutorialDesc(db, descType, newDesc);
          await clearState(db, ADMIN_ID);
          await sendMessage(chat_id, `✅ متن توضیحات <b>${descLabel}</b> با موفقیت ذخیره شد و از این پس همراه ویدیوی آموزشی ${descLabel} برای کاربران ارسال خواهد شد.`, settingsMenu());
          return new Response('OK');
        }
        ...
        if (user_id === ADMIN_ID && state && state.step === 'WAIT_WIPE_USER_TARGET') {
          ...
          const confirmKb = { inline_keyboard: [
            ...
            [{ text: "🎁 فقط تاریخچه تست این کاربر پاک شود", callback_data: `admwipeusertest_${row.user_id}` }],
            [{ text: "🎉 فقط تاریخچه جشنواره این کاربر پاک شود", callback_data: `admwipeuserfestival_${row.user_id}` }],
            [{ text: "❌ انصراف", callback_data: "admwipe_cancel" }]
          ] };
          await sendMessage(ADMIN_ID, `⚠️ <b>تایید حذف کاربر</b>\n\n👤 کاربر: ${userLink}\n🆔 آیدی: <code>${row.user_id}</code>\n📦 تعداد سرویس‌ها: ${svcCount}\n\nبا تایید گزینه اول، حساب کاربری، تمام سرویس‌ها، وضعیت فعلی و سوابق دعوت این کاربر برای همیشه پاک می‌شود.\nبا گزینه‌های دوم/سوم، فقط سابقه دریافت تست یا جشنواره این کاربر پاک می‌شود تا بتواند دوباره (بدون نیاز به صبر) دریافت کند؛ سایر اطلاعات او دست‌نخورده می‌ماند.\nآیا مطمئن هستید؟`, confirmKb);
          return new Response('OK');
        }
      ...
      if (update.callback_query) {
        ...
        if (data.startsWith('admwipeusertest_')) {
          if (user_id !== ADMIN_ID) return new Response('OK');
          const targetUid = data.replace('admwipeusertest_', '');
          await db.prepare("UPDATE users SET last_test_date = NULL, last_test_id = NULL WHERE user_id = ?").bind(targetUid).run();
          await callTelegram('answerCallbackQuery', { callback_query_id: call.id, text: "✅ تاریخچه تست کاربر پاک شد.", show_alert: true });
          await callTelegram('editMessageText', { chat_id, message_id: msg_id, text: `✅ تاریخچه اکانت تست کاربر <code>${targetUid}</code> پاک شد؛ این کاربر می‌تواند دوباره تست دریافت کند.`, parse_mode: "HTML" });
          return new Response('OK');
        }

        if (data.startsWith('admwipeuserfestival_')) {
          if (user_id !== ADMIN_ID) return new Response('OK');
          const targetUid = data.replace('admwipeuserfestival_', '');
          await db.prepare("UPDATE users SET last_festival_claim = NULL, last_festival_id = NULL WHERE user_id = ?").bind(targetUid).run();
          await callTelegram('answerCallbackQuery', { callback_query_id: call.id, text: "✅ تاریخچه جشنواره کاربر پاک شد.", show_alert: true });
          await callTelegram('editMessageText', { chat_id, message_id: msg_id, text: `✅ تاریخچه جشنواره کاربر <code>${targetUid}</code> پاک شد؛ این کاربر می‌تواند دوباره از همین جشنواره استفاده کند.`, parse_mode: "HTML" });
          return new Response('OK');
        }
        ...
        // ================= ویرایش متن توضیحاتِ همراهِ ویدیوهای آموزشی (جداگانه برای هر اپ) =================
        if (data === 'admtutdesc_edit_v2ray' || data === 'admtutdesc_edit_v2box') {
          if (user_id !== ADMIN_ID) return new Response('OK');
          const descType = data === 'admtutdesc_edit_v2ray' ? 'v2ray' : 'v2box';
          const descLabel = descType === 'v2ray' ? 'V2ray' : 'V2box';
          const curDesc = await getTutorialDesc(db, descType);
          await setState(db, ADMIN_ID, { step: 'WAIT_TUTORIAL_DESC', desc_type: descType });
          await callTelegram('answerCallbackQuery', { callback_query_id: call.id });
          await sendMessage(
            chat_id,
            `📝 <b>متن فعلی توضیحات ${descLabel}:</b>\n\n${escapeHtml(curDesc)}\n\n➖➖➖➖\n✏️ لطفاً متن جدیدی که می‌خواهید همراه ویدیوی آموزشی <b>${descLabel}</b> برای کاربران ارسال شود را تایپ و ارسال کنید.\n\n⚠️ توجه: به‌دلیل محدودیت تلگرام، طول کپشن ویدیو حداکثر ۱۰۲۴ کاراکتر می‌تواند باشد.`,
            pendingMenu()
          );
          return new Response('OK');
        }
        ...
        if (data === 'admtrial_new_round_ask') {
          if (user_id !== ADMIN_ID) return new Response('OK');
          const confirmKb = { inline_keyboard: [
            [{ text: "✅ بله، دوره جدید شروع شود", callback_data: "admtrial_new_round_do" }],
            [{ text: "❌ انصراف", callback_data: "admtrialcfg" }]
          ] };
          await callTelegram('answerCallbackQuery', { callback_query_id: call.id });
          await callTelegram('editMessageText', { chat_id, message_id: msg_id, text: "⚠️ <b>تایید شروع دوره جدید تست</b>\n\nبا این کار، آیدی دوره تست تغییر می‌کند و تاریخچه دریافت تست/جشنواره <b>همه کاربران</b> پاک می‌شود؛ یعنی همه دوباره می‌توانند مثل بار اول تست دریافت کنند. این عملیات غیرقابل بازگشت است. آیا مطمئن هستید؟", reply_markup: confirmKb, parse_mode: "HTML" });
          return new Response('OK');
        }

        if (data === 'admtrial_new_round_do') {
          if (user_id !== ADMIN_ID) return new Response('OK');
          const cfg = await getTrialConfig(db);
          cfg.trial_id = (cfg.trial_id || 1) + 1;
          await setSetting(db, 'trial_config', cfg);
          await db.prepare("UPDATE users SET last_test_date = NULL, last_test_id = NULL").run();
          const { msg, kb } = await renderTrialConfigMessage(db);
          await callTelegram('answerCallbackQuery', { callback_query_id: call.id, text: "✅ دوره جدید تست شروع شد.", show_alert: true });
          await callTelegram('editMessageText', { chat_id, message_id: msg_id, text: msg, reply_markup: kb, parse_mode: "HTML" });
          return new Response('OK');
        }
        ...
        if (data === 'admfestival_new_round_ask') {
          if (user_id !== ADMIN_ID) return new Response('OK');
          const confirmKb = { inline_keyboard: [
            [{ text: "✅ بله، جشنواره جدید شروع شود", callback_data: "admfestival_new_round_do" }],
            [{ text: "❌ انصراف", callback_data: "admfestivalcfg" }]
          ] };
          await callTelegram('answerCallbackQuery', { callback_query_id: call.id });
          await callTelegram('editMessageText', { chat_id, message_id: msg_id, text: "⚠️ <b>تایید شروع جشنواره جدید</b>\n\nبا این کار، آیدی جشنواره تغییر می‌کند و همه کاربران (حتی کسانی که از جشنواره قبلی استفاده کرده‌اند) می‌توانند از این جشنواره جدید استفاده کنند. این عملیات غیرقابل بازگشت است. آیا مطمئن هستید؟", reply_markup: confirmKb, parse_mode: "HTML" });
          return new Response('OK');
        }

        if (data === 'admfestival_new_round_do') {
          if (user_id !== ADMIN_ID) return new Response('OK');
          const cfg = await getFestivalConfig(db);
          cfg.festival_id = (cfg.festival_id || 1) + 1;
          await setSetting(db, 'festival_config', cfg);
          const { msg, kb } = await renderFestivalConfigMessage(db);
          await callTelegram('answerCallbackQuery', { callback_query_id: call.id, text: `✅ جشنواره جدید (آیدی #${cfg.festival_id}) شروع شد.`, show_alert: true });
          await callTelegram('editMessageText', { chat_id, message_id: msg_id, text: msg, reply_markup: kb, parse_mode: "HTML" });
          return new Response('OK');
        }
        ...
        // ================= تایید پرداختی / تست توسط ادمین =================
          ...
          if (preSelectedDomain) {
              ...
              if (cfRes.success && cfRes.subLink) {
                  ...
                  if (action === 'test') {
                    const trialCfgApply2 = await getTrialConfig(db);
                    await db.prepare("INSERT INTO users (user_id, last_test_date, last_test_id) VALUES (?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET last_test_date = excluded.last_test_date, last_test_id = excluded.last_test_id").bind(targetUser, getShamsiDateOnly(), trialCfgApply2.trial_id || 1).run();
                  } else if (action === 'festival') {
                    const festivalCfgApply2 = await getFestivalConfig(db);
                    // جشنواره سقف مشترک با تست دارد: last_test_date هم به‌روزرسانی می‌شود تا فاصله مجاز مشترک اعمال شود
                    await db.prepare("INSERT INTO users (user_id, last_test_date, last_festival_claim, last_festival_id) VALUES (?, ?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET last_test_date = excluded.last_test_date, last_festival_claim = excluded.last_festival_claim, last_festival_id = excluded.last_festival_id").bind(targetUser, getShamsiDateOnly(), getShamsiDateOnly(), festivalCfgApply2.festival_id || 1).run();
                  }
