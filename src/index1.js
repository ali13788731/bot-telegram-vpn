...
function adminPanelMenu() {
  ...
  return { 
    keyboard: [
      ...
      [{ text: "📢 ارسال اطلاعیه به همه کاربران" }, { text: "⚙️ تنظیمات ربات" }]
    ], 
...
const FIXED_MENU_BUTTON_TEXTS = new Set([
  ...
  "📢 ارسال اطلاعیه به همه کاربران", "⚙️ تنظیمات ربات", "🗑 پاک کردن کامل دیتابیس", "🔙 بازگشت به پنل مدیریت",
    ...
    try {
      if (update.message) {
        ...
        if (text === "📖 راهنمای پنل مدیریت" && user_id === ADMIN_ID) {
          ...
          `4️⃣ <b>ارسال اطلاعیه همگانی:</b> از دکمه «📢 ارسال اطلاعیه به همه کاربران» استفاده کنید. هر نوع پیام (متن، عکس، لینک و ...) که ارسال کنید، پس از تایید شما عیناً برای تمام کاربران ربات فرستاده می‌شود.\n\n` +
        ...
        // ================= ارسال اطلاعیه همگانی به تمام کاربران =================
        if (text === "📢 ارسال اطلاعیه به همه کاربران" && user_id === ADMIN_ID) {
          await setState(db, ADMIN_ID, { step: 'WAIT_BROADCAST_CONTENT' });
          await sendMessage(chat_id, "📢 <b>ارسال اطلاعیه همگانی</b>\n\nهر چیزی که می‌خواهید برای <b>تمام کاربران ربات</b> ارسال شود را همینجا بفرستید؛ متن (همراه با فرمت و لینک)، عکس همراه با کپشن یا هر نوع پیام دیگر پشتیبانی می‌شود.\n\nپس از ارسال، پیش‌نمایش را برای تایید نهایی خواهید دید.", pendingMenu());
          return new Response('OK');
        }

        if (user_id === ADMIN_ID && state && state.step === 'WAIT_BROADCAST_CONTENT') {
          await setState(db, ADMIN_ID, { step: 'CONFIRM_BROADCAST', src_chat_id: chat_id, src_message_id: msg.message_id });
          await sendMessage(ADMIN_ID, "👇 این پیش‌نمایش دقیقاً همانطور که هست (متن، فرمت، عکس، لینک) برای تمام کاربران ارسال خواهد شد:");
          const confirmKb = {
            inline_keyboard: [
              [{ text: "✅ ارسال برای همه کاربران", callback_data: "confirm_broadcast" }],
      ...
      if (update.callback_query) {
        ...
        // ================= تایید/لغو ارسال اطلاعیه همگانی =================
        if (data === 'confirm_broadcast') {
          ...
          if (allUsers && allUsers.length > 0) {
            for (const u of allUsers) {
              try {
                const res = await callTelegram('copyMessage', { chat_id: u.user_id, from_chat_id: srcChatId, message_id: srcMsgId });
